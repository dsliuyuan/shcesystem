			...
			<tr>
				...
				<shiro:hasPermission name="sys:role:edit"><td>
					<c:if test="${(role.sysData eq fns:getDictValue('是', 'yes_no', '1') && fns:getUser().admin)||!(role.sysData eq fns:getDictValue('是', 'yes_no', '1'))}">
						<a href="${ctx}/sys/role/form?id=${role.id}" class="button btn-modify">修改</a>
					</c:if>
					<a href="${ctx}/sys/role/assign?id=${role.id}" class="button btn-show1">分配</a>
					<a href="${ctx}/sys/role/delete?id=${role.id}" onclick="return confirmx('确认要删除该角色吗？', this.href)" class="button btn-delete">删除</a>
