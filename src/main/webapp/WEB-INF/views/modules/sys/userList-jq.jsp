...
<body>
		...
		<c:forEach items="${page.list}" var="user">
			<tr>
				...
				<shiro:hasPermission name="sys:user:edit"><td>
    				<a href="${ctx}/sys/user/form?id=${user.id}" class="button btn-modify">修改</a>
					<a href="${ctx}/sys/user/delete?id=${user.id}" onclick="return confirmx('确认要删除该用户吗？', this.href)" class="button btn-delete">删除</a>
	...
	<script type="text/javascript">
		...
		$('#dataGrid').dataGrid({
			...
			// 设置数据表格列
			columnModel: [
				{header:'归属机构', name:'loginName', index:'login_name', width:100, frozen:true , formatter: function(val, obj, row, act){
					return '<a href="${ctx}/sys/user/form?id='+row.id+'" class="btnList" data-title="编辑用户">'+val+'</a>';
				}},
				{header:'归属机构', name:'company.name', index:'', width:160},
