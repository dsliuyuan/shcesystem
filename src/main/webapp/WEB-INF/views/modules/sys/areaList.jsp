	...
	<script type="text/template" id="treeTableTpl">
		<tr id="{{row.id}}" pId="{{pid}}">
			...
			<shiro:hasPermission name="sys:area:edit"><td>
				<a href="${ctx}/sys/area/form?id={{row.id}}" class="button btn-modify">修改</a>
				<a href="${ctx}/sys/area/form?parent.id={{row.id}}" class="button btn-show3">添加下级区域</a>
				<a href="${ctx}/sys/area/delete?id={{row.id}}" onclick="return confirmx('要删除该区域及所有子区域项吗？', this.href)" class="button btn-delete">删除</a>
